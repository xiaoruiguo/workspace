...
func_create_dirs $CERTS $CRL $NEWCERTS $PRIVATE
touch $INDEXFILE
...
commonName_default              = root.ca.lijiaocn
commonName_max                  = 64
...
#-x509表示生成自自签署的证书，而非签署请求
openssl req -config $CONF -key $KEY -new -x509 -days 365 -sha256 -extensions v3_ca -out $CERT
